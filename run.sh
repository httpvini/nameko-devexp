...
if [ -n "${AMQP_URI}" ]; then
    ...
    AMQP_URI=$(echo ${VCAP_SERVICES} | jq -r '.Rabbit[0].credentials.uri')
    POSTGRES_URI=$(echo ${VCAP_SERVICES} | jq -r '.Postgresql[0].credentials.uri')

    REDIS_HOST=$(echo ${VCAP_SERVICES} | jq -r '.Redis[0].credentials.hostname')
    REDIS_PORT=$(echo ${VCAP_SERVICES} | jq -r '.Redis[0].credentials.port')
    REDIS_PWD=$(echo ${VCAP_SERVICES} | jq -r '.Redis[0].credentials.password')
    REDIS_URI=redis://:${REDIS_PWD}@${REDIS_HOST}:${REDIS_PORT}
